...
const Home = () => {
  ...
  useEffect(() => {
    const fetchLatestPosts = async () => {
      try {
        const response = await api.get('/posts?status=published&limit=11&order=createdAt.desc');
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative font-sans">
      ...
      <section className="relative bg-primary-900 text-white h-[450px] flex items-center overflow-hidden">
        ...
        <div className="container mx-auto px-4 z-10 relative grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-6">
            <span className="bg-secondary-600 text-white px-3 py-1 text-xs font-bold uppercase tracking-wider rounded">Thông điệp tuần</span>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-lg shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-white/10 pb-2">
                  <span>Lượt khám bệnh</span><span className="font-bold text-2xl">12,450</span>
                </div>
                <div className="flex justify-between items-center border-b border-white/10 pb-2">
                  <span>Hồ sơ mới lập</span><span className="font-bold text-2xl text-secondary-400">842</span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-white/10">
                  <span>Xe cấp cứu đang chạy</span><span className="font-bold text-2xl text-orange-400">45</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative w-full h-[250px] overflow-hidden container mx-auto px-4 mt-8 rounded-2xl shadow-xl">
        <img
        ...
        <div className="absolute inset-0 flex items-center justify-between p-4">
          <button onClick={goToPrevious} className="bg-black/50 text-white w-10 h-10 rounded-full hover:bg-black/75 transition ml-2 flex items-center justify-center">
            <ChevronLeft />
          </button>
          <button onClick={goToNext} className="bg-black/50 text-white w-10 h-10 rounded-full hover:bg-black/75 transition mr-2 flex items-center justify-center">
            <ChevronRight />
          </button>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tighter flex items-center gap-2">
                   <Zap className="text-red-600 fill-current" size={24} /> TIÊU ĐIỂM Y TẾ
                </h3>
              </div>
              ...
              {loading ? (
                ...
                <Link to={`/news/detail/${spotlightPost.id}`} className="group block">
                  <div className="relative aspect-video rounded-2xl overflow-hidden shadow-2xl mb-6">
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                      <span className="bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-sm uppercase tracking-widest mb-3 inline-block">
                        {SERVICE_CATEGORIES.find(c => c.id === spotlightPost.category)?.title || "TIN TỨC"}
                      </span>
                  ...
                  <div className="flex items-center gap-4 mt-4 text-xs font-bold text-gray-400">
                     <span className="flex items-center gap-1"><Clock size={14}/> {new Date(spotlightPost.createdAt).toLocaleDateString('vi-VN')}</span>
                     <span className="text-red-600 hover:underline flex items-center gap-1">Xem chi tiết <ArrowRight size={14}/></span>
                  </div>
                </Link>
              )}
            </div>
            ...
            <div className="lg:col-span-5">
              <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-2">
                <h3 className="text-lg font-black text-gray-800 uppercase tracking-tighter">MỚI CẬP NHẬT</h3>
                <Link to="/news/events" className="text-xs font-bold text-primary-600 hover:underline">Tất cả bài viết</Link>
              </div>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h3 className="text-3xl font-black text-primary-900 uppercase tracking-tight mb-4">Dịch vụ & Tiện ích</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {SERVICE_CATEGORIES.map((category) => {
              ...
              return (
                ...
                >
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${category.iconBoxClass}`}>
                    <Icon size={28} />
                  </div>
                  <h4 className={`font-bold text-sm uppercase leading-tight ${category.titleClass}`}>
                    {category.title}
                  </h4>
                </Link>
              );
