      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl flex-grow">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
           <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div>
                 <p className="text-gray-400 text-[10px] font-black uppercase">Tổng bài viết</p>
                 <h3 className="text-2xl font-black text-gray-800">{posts.length}</h3>
              </div>
           </div>
           <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div>
                 <p className="text-gray-400 text-[10px] font-black uppercase">Đã xuất bản</p>
                 <h3 className="text-2xl font-black text-gray-800">{posts.filter(p => p.status === 'published').length}</h3>
              </div>
           </div>
           <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div>
                 <p className="text-gray-400 text-[10px] font-black uppercase">Bản nháp</p>
                 <h3 className="text-2xl font-black text-gray-800">{posts.filter(p => p.status === 'draft').length}</h3>
              </div>
           </div>
          ...
          <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50/50">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
               ...
               >
                 ...
                 {SERVICE_CATEGORIES.map(cat => <option key={cat.id} value={cat.id}>{cat.title}</option>)}
               </select>
            </div>
          </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-20 text-center">
                       ...
                       <p className="text-gray-400 font-bold uppercase text-[10px]">Đang tải dữ liệu bài viết...</p>
                    </td>
                  </tr>
                  ...
                  return (
                    <tr key={post.id} className="hover:bg-gray-50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="min-w-0">
                            <h4 className="font-bold text-gray-800 text-sm truncate max-w-xs">{post.title}</h4>
                            <p className="text-[10px] text-gray-400 truncate max-w-xs italic">{post.summary}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${post.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                           <div className={`w-1.5 h-1.5 rounded-full ${post.status === 'published' ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col text-[11px] text-gray-500 font-medium">
                           ...
                           <span className="flex items-center gap-1 mt-0.5"><Clock size={12}/> {new Date(post.createdAt).toLocaleTimeString('vi-VN', {hour: '2-digit', minute: '2-digit'})}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
