    ...
    categories.forEach((cat) => {
      ...
      data[cat.key] = {
        ...
        tiepNhan: [
          {
            stt: 1,
            noiDung: "Đơn vị báo cáo",
            soLuong: reportedCount,
            tyLe: formatRate(reportedCount, totalUnits),
          },
          {
            stt: 2,
            noiDung: "Đơn vị không báo cáo",
            soLuong: notReportedCount,
            tyLe: formatRate(notReportedCount, totalUnits),
          },
        ],
        deCuong: [
          {
            stt: 1,
            noiDung: "Đơn vị báo cáo đúng theo đề cương và biểu mẫu",
            soLuong: onTimeCount,
            tyLe: formatRate(onTimeCount, reportedCount),
          },
          {
            stt: 2,
            noiDung: "Đơn vị báo cáo không đúng theo đề cương và biểu mẫu",
            soLuong: lateCount,
            tyLe: formatRate(lateCount, reportedCount),
          },
        ],
      };
        ...
        const facilityNames: string[] = items.map((fb) => {
          if (fb.info) {
            ...
            const firstMatchedField = Object.entries(fb.info)
              .filter(([k]) => !isNaN(Number(k)))
              .find(
                ([_, v]: [string, any]) => v?.value?.key && v?.value?.value,
              );
          ...
          const facility = facilities.find(
            (f: any) => String(f.id) === String(facilityId),
          );
          return facility
            ? facility.name
            : fb.fullName || fb.name || `Đơn vị (${facilityId || "?"})`;
        });
        data.phuLuc.push({
          nhom: cat.nhom,
