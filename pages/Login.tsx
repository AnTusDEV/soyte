...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Đảm bảo email đúng định dạng nếu người dùng chỉ nhập "admin"
      ...
      switch (err.code) {
        ...
        case "auth/too-many-requests":
          setError("Quá nhiều lần thử thất bại. Vui lòng thử lại sau.");
          break;
        default:
          setError(
            "Lỗi đăng nhập: " +
              (err.message || "Vui lòng kiểm tra lại cấu hình Firebase.")
          );
      }
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className=" bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
        <Link
          to="/"
          className="inline-flex items-center text-sm text-gray-500 hover:text-primary-600 mb-6 transition"
        >
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          ...
          <div className="bg-gray-50 p-6 border-t border-gray-100 text-center">
            <p className="text-xs text-gray-400 font-medium italic">
              Cổng đăng nhập bảo mật cho cán bộ ngành Y tế Thủ đô.
            </p>
          </div>
        </div>
      </div>
